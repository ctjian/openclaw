...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    `For long waits, avoid rapid poll loops: use ${execToolName} with enough yieldMs or ${processToolName}(action=poll, timeout=<ms>). Prefer short-to-moderate poll timeouts (about 2,000-10,000ms) instead of single very long waits (for example, 10,000ms).`,
